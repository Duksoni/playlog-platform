...
impl GameEntityRepository for PostgresGameEntityRepository {
    async fn get(&self, id: i32) -> Result<Option<GameEntity>> {
        ...
        let result = query_as::<_, GameEntity>(AssertSqlSafe(query))
            .bind(id)
    ...
    async fn get_all(&self, page: u64, limit: u64) -> Result<GameEntityPagedResponse> {
        ...
        let data = query_as::<_, GameEntitySimple>(AssertSqlSafe(query))
            .bind(limit)
            .bind(offset)
            .fetch_all(&self.pool)
            .await?;
    ...
    async fn exists(&self, id: i32) -> Result<()> {
        ...
        let exists = query_scalar::<_, bool>(AssertSqlSafe(query))
            .bind(id)
            .fetch_optional(&self.pool)
            .await?
            .unwrap_or(false);
