...
#[utoipa::path(
    ...
    params(PagedQuery),
    responses(
        (status = 200, description = "List of developers", body = GameEntityPagedResponse),
...
async fn get_all_paged(
    ...
    let result = state.developer_repository.get_all(query.page, query.limit).await?;
    Ok(Json(result))
}
...
async fn search(
    ...
    query.validate().map_err(ApiError::from)?;
    let result = state.developer_repository.find_by_name(&query.q, query.limit).await?;
    Ok(Json(result))
}
...
async fn create(
    ...
    let result = state.developer_repository.create(name).await?;
