...
#[utoipa::path(
    ...
    params(PagedQuery),
    responses(
        (status = 200, description = "List of platforms", body = GameEntityPagedResponse),
...
async fn get_all_paged(
    ...
    let result = state.platform_repository.get_all(query.page, query.limit).await?;
    Ok(Json(result))
}
...
async fn search(
    ...
    query.validate().map_err(ApiError::from)?;
    let result = state.platform_repository.find_by_name(&query.q, query.limit).await?;
    Ok(Json(result))
}
...
async fn create(
    ...
    let result = state.platform_repository.create(name).await?;
...
#[utoipa::path(
    ...
    responses(
        ...
        (status = 403, description = "Forbidden"),
        (status = 404, description = "Platform not found"),
        (status = 409, description = "Conflict - in use"),
        (status = 422, description = "Invalid path/query/body"),
    ),
    tag = "platforms",
    security(("bearer" = [])),
